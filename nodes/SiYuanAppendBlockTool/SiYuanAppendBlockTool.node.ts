...
export class SiYuanAppendBlockTool implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'SiYuan Tool: Append Block',
		name: 'siYuanAppendBlockTool',
		group: ['Tool', 'SiYuan'],
		version: 1,
		description: 'AI Tool: Appends Markdown content to the end of a specified parent block (document or block) in SiYuan.',
		defaults: {
			name: 'SiYuan Append Block',
		},
		inputs: ['main'],
		outputs: ['main'],
		credentials: [
		...
		properties: [
			// --- Input Properties for this specific tool ---
			{
				...
				required: true,
				default: '',
				description: 'The ID of the document or block to append content to',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		try {
			...
			const result = await client.appendBlock(parentBlockId, markdownContent);
